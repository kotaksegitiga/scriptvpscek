...
T   debconf-set-selections <<<"keyboard-configuration keyboard-configuration/model select Generic 105-key (Intl) PC"
...
i   debconf-set-selections <<<"keyboard-configuration keyboard-configuration/unsupported_layout boolean true"
...
  t wget -q -O /etc/haproxy/haproxy.cfg "${REPO}config/haproxy.cfg" >/dev/null 2>&1
    
...
t   Jika kamu bandingkan dengan nikmat yang Allah SWT berikan padamu.</p>
...
T   <p>Semoga kita termasuk orang-orang beruntung yang mendapatkan Rahmat dan Kasih Sayang Allah SWT.</p>
